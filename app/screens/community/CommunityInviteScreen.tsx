...
import {showCommunityToast} from './communityUx';
...
export default function CommunityInviteScreen() {
  ...
  const accept = async () => {
    ...
    try {
      ...
      showCommunityToast('Invite accepted. You are in.');
      navigation.replace('CommunityHome', {community});
    } catch (err: any) {
      showCommunityToast(err?.response?.data?.error || 'Invite did not work. Check the code.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <KeyboardAvoidingView {...keyboardAvoidingProps}>
        <ScrollView
          ...
          <Text style={styles.copy}>Paste the private invite code a moderator shared with you. Your public profile stays hidden.</Text>
          <TextInput
