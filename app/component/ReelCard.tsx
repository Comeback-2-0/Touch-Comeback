...
const ReelCard: React.FC<Props> = ({ id, uri, isActive, caption, user, reelId  }) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        ...
        const res = await axios.get(`http://192.168.55.73:3333/api/reels/${reelId}/comments`);
 ...
 const handleAddComment = async () => {
  if (commentInput.trim()) {
    try {
      const res = await axios.post(`http://192.168.55.73:3333/api/reels/${reelId}/comments`, {
        text: commentInput,
      });
